      ...
      res
        .then(async ({ data }: any) => {
          if (data.state) {
            ...
            for (let i = 0; i < data.genInfo.url.length; ++i){
              try{
                let bLoop = true
                while (bLoop) {
                  ...
                  fetch(data.genInfo.url[i]).then(res =>
                    {
                        if (res.status == 200) {
                          bLoop =false
                        }
        ...
        <Grid item xs={12}>
          ...
          >
            <ToggleButton value={false} aria-label='staging'>
              ...
              {
                methodValue === 'sketch2img' ? 'Re-デザイン' : 'ステージングプロ'
            ...
            <ToggleButton value={true} aria-label='redesign'>
              <Typography sx={{ fontWeight: 500 }}>
              {
               methodValue === 'sketch2img' ?  'ステージングプロ' :'Re-デザイン'
