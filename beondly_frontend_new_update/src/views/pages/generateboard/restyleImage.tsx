...
import { getAllProject, getRestyleProject } from 'src/pages/api/dashboardService'
...
const RestyleImage = ({
  ...
  useEffect(() => {
    ...
    if (loginuserData != null) {
      ...
      const res = getRestyleProject(userId)
      res
        .then(({ data }: any) => {
              ...
              {generatedImage.map((item: any, i: number) => (
                <OriginRadioImg
                  ...
                  data={{
                    ...
                    value: item.id,
                    prompt: item.prompt,
                    img: item.baseUrl ? item.baseUrl : item.url[0],
