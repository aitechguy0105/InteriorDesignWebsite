 ...
 * @param stack
 */
export const formatHistory = (stack: ELEMENT_INSTANCE[], state: HistoryState, version: string) => {
  if (state?.currentLineColor) {
    state.currentLineColor = Array.isArray(state?.currentLineColor) ? state.currentLineColor : [state.currentLineColor]
  }
  stack.forEach(ele => {
